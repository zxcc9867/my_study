...
class Saving_account(BankAccount):
    ...
    def info(self, name):
        print(
            f"""{self.name}님의 계좌 잔고는 {self._money}이며,이자율은 {0.05}이며
              출금제한 여부는 {
                  "제한되어 있습니다." if self._account_restriction == True else "출금할 수 있습니다." } """
        )
